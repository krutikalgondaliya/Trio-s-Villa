...
$customers_result = $conn->query("SELECT * FROM guest_users ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Customer Registry - Admin Panel</title>
...
<body class="bg-gray-100 font-sans min-h-screen flex">
    ...
    <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col fixed h-full z-20 border-r border-slate-800 shadow-xl">
        ...
        <nav class="flex-1 overflow-y-auto py-3 px-3 space-y-1 custom-scrollbar">
            ...
            <a href="customers.php" class="flex items-center space-x-3 px-3.5 py-2.5 rounded-lg transition text-xs font-semibold sidebar-link-active group">
                <i class="fa-solid fa-users w-5 text-center transition"></i> 
                <span>Customers</span>
            </a>
        ...
        <div>
            <header class="bg-white border-b px-8 py-4 flex justify-between items-center shadow-sm relative">
                <div>
                    ...
                    <p class="text-xs text-gray-500 font-medium mt-0.5">Profiles of visitors with accounts</p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 text-gray-600 text-xs font-semibold uppercase border-b">
                                <th class="p-4">Profile Avatar</th>
                        ...
                        <tbody class="divide-y text-sm">
                            <?php if ($customers_result && $customers_result->num_rows > 0): ?>
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="p-4">
                                        <?php if(!empty($c['profile_pic']) && file_exists("../uploads/" . $c['profile_pic'])): ?>
                                            <img src="../uploads/<?php echo htmlspecialchars($c['profile_pic']); ?>" class="w-9 h-9 rounded-full object-cover border">
                                        <?php else: ?>
                                            <div class="w-9 h-9 bg-blue-100 text-blue-600 font-bold rounded-full flex items-center justify-center text-xs border border-blue-200">
                                                <?php echo strtoupper(substr($c['fullname'], 0, 1)); ?>
                                            </div>
                                        <?php endif; ?>
                                    </td>
                                    <td class="p-4 font-bold text-gray-900"><?php echo htmlspecialchars($c['fullname']); ?></td>
                                    <td class="p-4 text-gray-600 font-mono text-xs"><?php echo htmlspecialchars($c['email']); ?></td>
                                    <td class="p-4 text-xs text-gray-400 font-medium"><?php echo date('d M Y', strtotime($c['created_at'])); ?></td>
                                    <td class="p-4 text-center">
                                        <a href="customer_history.php?email=<?php echo urlencode($c['email']); ?>" class="text-blue-600 hover:text-blue-800 font-bold text-xs">View History</a>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="p-8 text-center text-gray-400">No registered guest accounts detected.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
