...
// Fetch payment transactions based on approved bookings
$payments_result = $conn->query("
    SELECT b.id as booking_id, b.guest_name, b.guest_email, b.check_in, b.check_out, r.room_number, r.price_per_night 
    FROM bookings b
...
<head>
    ...
    <title>Payments Ledger - Admin Panel</title>
...
<body class="bg-gray-100 font-sans min-h-screen flex">
    ...
    <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col fixed h-full z-20 border-r border-slate-800 shadow-xl">
        ...
        <nav class="flex-1 overflow-y-auto py-3 px-3 space-y-1 custom-scrollbar">
            ...
            <a href="payments.php" class="flex items-center space-x-3 px-3.5 py-2.5 rounded-lg transition text-xs font-semibold sidebar-link-active group">
                <i class="fa-solid fa-credit-card w-5 text-center transition"></i> 
                <span>Payments</span>
            </a>
    ...
    <div class="flex-1 flex flex-col pl-64 justify-between min-h-screen">
        <div>
            <header class="bg-white border-b px-8 py-4 flex justify-between items-center shadow-sm relative">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">Payments Ledger</h1>
                    <p class="text-xs text-gray-500 font-medium mt-0.5">Track financial transactions</p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 text-gray-600 text-xs font-semibold uppercase border-b">
                                <th class="p-4">Transaction Reference</th>
                                <th class="p-4">Guest Details</th>
                                <th class="p-4">Room</th>
                                <th class="p-4">Total Settled</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y text-sm">
                            <?php if ($payments_result && $payments_result->num_rows > 0): ?>
                                <?php while($p = $payments_result->fetch_assoc()): 
                                    $days = (int) (new DateTime($p['check_in']))->diff(new DateTime($p['check_out']))->days;
                                    $days = $days > 0 ? $days : 1;
                                    $total_paid = $days * $p['price_per_night'];
                                ?>
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="p-4 text-xs font-mono text-gray-500">TXN-<?php echo str_pad($p['booking_id'], 6, '0', STR_PAD_LEFT); ?></td>
                                    <td class="p-4">
                                        <div class="font-bold text-gray-900"><?php echo htmlspecialchars($p['guest_name']); ?></div>
                                        <div class="text-xs text-gray-400"><?php echo htmlspecialchars($p['guest_email']); ?></div>
                                    </td>
                                    <td class="p-4 font-semibold text-blue-600">Room <?php echo htmlspecialchars($p['room_number']); ?></td>
                                    <td class="p-4 font-bold text-emerald-600">₹<?php echo number_format($total_paid, 2); ?></td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="4" class="p-8 text-center text-gray-400">No transactions recorded yet.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
